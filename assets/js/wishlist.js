 ...
 *
 * Vanilla JS (no jQuery dependency of our own). Event delegation so buttons
 * rendered after page load (infinite scroll, quick-view, AJAX filters) also
 * work. Config is provided by the storefront-kit WishlistEngine via
 * wp_localize_script as `window.shortlistWishlist`.
 */
document.addEventListener('DOMContentLoaded', function () {
  document.addEventListener('click', function (event) {
    var button = event.target.closest('[data-shortlist-wishlist-button]');
    ...
    fetch(config.ajaxUrl, {
      ...
      document.querySelectorAll(
        '[data-shortlist-wishlist-button][data-product-id="' + productId + '"]'
      ).forEach(function (el) {
